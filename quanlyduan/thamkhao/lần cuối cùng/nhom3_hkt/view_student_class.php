<?php 
    $id_class = $_GET['id'];
    // echo $id_class;
    // $qry = $conn->query("SELECT concat(c.level,'-',c.section) as class FROM classes c WHERE id like ".$id_class.";");
	// $row_c= $qry->fetch_assoc();
    // echo $row_c['class'];
?>
		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT r.*, s.class_id as id_class ,concat(s.firstname,' ',s.middlename,' ',s.lastname) as name,s.student_code,concat(c.level,'-',c.section) as class FROM results r inner join classes c on c.id = r.class_id inner join students s on s.id = r.student_id $where and s.class_id=".$id_class." order by unix_timestamp(r.date_created) desc ");
					...
					<tr>
                        ...
                        <td><b><?php echo ucwords($row['id_class']) ?></b></td>
						<td class="text-center"><b><?php if(isset($subjects)) echo $subjects; else echo 0; ?></b></td>
						<td class="text-center"><b><?php if(isset($row['marks_percentage'])) echo $row['marks_percentage']; else echo 0  ?></b></td>
